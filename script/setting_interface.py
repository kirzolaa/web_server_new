...
class SettingInterface(ScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        # setting label
        self.settingLabel = QLabel(self.tr("Settings: Heavily work-in-progress (preview)"), self)
    ...
    def __setQss(self):
        ...
        qss_path = os.path.join(base_path, 'resource_settings', 'qss', theme, 'setting_interface.qss')
        
        try:
            with open(qss_path, encoding='utf-8') as f:
                self.setStyleSheet(f.read())
        except FileNotFoundError:
            print(f"Warning: Could not find QSS file at {qss_path}")
            # Set a basic style as fallback
            self.setStyleSheet("""
